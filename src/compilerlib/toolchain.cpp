...
namespace compilerlib
{
    namespace
    {
        ...
        CT_NODISCARD bool isCxxLang(llvm::StringRef lang)
        {
            ...
            if (lang.starts_with("="))
                lang = lang.substr(1);
            return lang == "c++" || lang == "c++-header" || lang == "c++-cpp-output" ||
                   lang == "objective-c++" || lang == "objective-c++-header";
        }
        ...
        CT_NODISCARD bool isCxxSourceExt(llvm::StringRef ext)
        {
            ...
            return lower == ".cc" || lower == ".cpp" || lower == ".cxx" || lower == ".c++" ||
                   lower == ".cp" || lower == ".mm";
        }
        ...
        CT_NODISCARD bool isSourceExt(llvm::StringRef ext)
        {
            if (ext.empty())
                return false;
            std::string lower = ext.lower();
            return lower == ".c" || lower == ".cc" || lower == ".cpp" || lower == ".cxx" ||
                   lower == ".c++" || lower == ".cp" || lower == ".m" || lower == ".mm";
        }
        ...
        CT_NODISCARD bool looksLikeCxxSymbol(llvm::StringRef name)
        {
            ...
            if (name.starts_with("__cxa") || name.starts_with("___cxa"))
                return true;
            if (name.starts_with("__gxx_personality_v0") ||
        ...
        CT_NODISCARD bool binaryHasCxxSymbols(llvm::object::Binary& binary)
        {
            ...
            if (auto* arch = llvm::dyn_cast<llvm::object::Archive>(&binary))
            {
                ...
                {
                    llvm::Expected<std::unique_ptr<llvm::object::Binary>> childBin =
                        child.getAsBinary();
                    if (!childBin)
                    {
        ...
        {
            return arg == "-o" || arg == "-x" || arg == "-target" || arg == "--target" ||
                   arg == "-gcc-toolchain" || arg == "-isysroot" || arg == "-I" ||
                   arg == "-isystem" || arg == "-iquote" || arg == "-idirafter" ||
                   arg == "-iprefix" || arg == "-iwithprefix" || arg == "-iwithprefixbefore" ||
                   arg == "-include" || arg == "-imacros" || arg == "-D" || arg == "-U" ||
                   arg == "-L" || arg == "-F" || arg == "-MF" || arg == "-MT" || arg == "-MQ" ||
                   arg == "-Xclang" || arg == "-Xlinker" || arg == "-Xassembler" ||
                   arg == "-Xpreprocessor" || arg == "-Wl,";
        }
            ...
            if (auto found = llvm::sys::findProgramByName("xcrun"))
            {
                ...
                if (rc == 0)
                {
                    llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>> buf =
                        llvm::MemoryBuffer::getFile(outPath);
                    if (buf && *buf)
                    {
                                          ...
                                          std::string& error)
    {
        ...
        if (!scan.has_source_inputs && scan.has_object_inputs && !out.force_cxx_driver)
        {
            for (const auto& path : scan.inputs)
            {
                ...
                if (!looksCxx)
                {
                    error = "failed to inspect object: " + path + ": " +
                            llvm::toString(looksCxx.takeError());
                    return false;
                }
                if (*looksCxx)
                {
